...
近期提交记录使用 `feature:`、`refactor:` 和 `docs:` 等小写前缀。提交标题使用中文，应简短，并且一次提交只聚焦一项改动。PR 应说明对用户可见的影响，列出已执行的命令或手动检查步骤，关联相关 issue；涉及渲染进程或桌面界面的改动时应附带截图。若改动涉及 `native/`、应用打包流程或 preload/API 边界，必须明确说明，因为这些内容会影响发布构建和安全审查。
