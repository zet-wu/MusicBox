...
describe('LyricsLoaderController', () => {
    ...
    it('不同文件即使元数据相同也分别加载歌词', async () => {
        ...
        const load = vi.fn().mockResolvedValue({document: createDocument('歌词')});
    ...
    it('快速切歌时中止并丢弃较早返回的歌词', async () => {
        ...
        const load = vi.fn()
            ...
            .mockResolvedValueOnce({document: createDocument('第二首')});
        const setDocument = vi.fn();
        const controller = new LyricsLoaderController({
            setDocument,
            showLoading: vi.fn(),
        ...
        await controller.loadLyrics(createTrack('file-b', 'B.flac'));
        resolveFirst({document: createDocument('第一首')});
        await firstLoad;

        expect(setDocument).toHaveBeenCalledOnce();
        expect(setDocument).toHaveBeenCalledWith(createDocument('第二首'));
