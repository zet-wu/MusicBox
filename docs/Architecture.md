...
侧边栏“文件夹”页面是目录来源的统一用户管理入口，只展示 `directory` 来源；精确文件来源继续由系统维护。卡片列表通过轻量目录概览接口读取歌曲数、扫描时间和歌单绑定摘要，避免把完整 `knownFiles` 清单传入 renderer；进入文件夹详情时再通过按来源查询接口返回当前索引中属于该来源的歌曲。详情页复用统一的只读歌曲集合交互，支持选择、播放、收藏和歌曲上下文菜单。

文件夹侧和歌单侧分别使用面向各自上下文的绑定弹窗，但共享同一套绑定、恢复排除和解绑业务语义。文件夹卡片还可复用标准创建歌单弹窗，以文件夹名预填标题；歌单创建成功后调用已有来源绑定命令，由绑定同步生成派生成员。若绑定失败，已创建歌单会保留并向用户报告部分失败，不执行跨缓存文件的自动回滚。设置页只保留自动扫描计划及进入文件夹页面的跳转入口。
