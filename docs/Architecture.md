...
侧边栏“文件夹”页面是目录来源的统一用户管理入口，只展示 `directory` 来源；精确文件来源继续由系统维护。页面通过轻量目录概览接口读取歌曲数、扫描时间和歌单绑定摘要，避免把完整 `knownFiles` 清单传入 renderer。文件夹侧和歌单侧分别使用面向各自上下文的绑定弹窗，但共享同一套绑定、恢复排除和解绑业务语义。设置页只保留自动扫描计划及进入文件夹页面的跳转入口。
